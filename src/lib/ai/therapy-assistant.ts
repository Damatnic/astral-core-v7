...
export class AITherapyAssistant {
  ...
  private generateInsights(patterns: string[], sessionData: any): string[] {
    ...
    if (patterns.includes('all-or-nothing')) {
      insights.push('You may be experiencing all-or-nothing thinking, which can amplify negative feelings.');
    }
    ...
    if (patterns.includes('high-anxiety')) {
      insights.push('Your anxiety levels appear elevated. This is affecting your thought patterns.');
    }

    if (sessionData.mood <= 3 && sessionData.anxiety >= 7) {
      insights.push('The combination of low mood and high anxiety suggests you may benefit from grounding techniques.');
    }
  ...
  private generateRecommendations(patterns: string[], riskLevel: CrisisSeverity): string[] {
    ...
    if (riskLevel === 'HIGH' || riskLevel === 'CRITICAL') {
      recommendations.push('Please consider reaching out to your therapist or crisis support immediately.');
  ...
  async generateTherapeuticResponse(
    ...
    if (sentiment === 'negative') {
      response = "I hear that you're going through a difficult time. Your feelings are valid, and it's okay to feel this way. Can you tell me more about what's contributing to these feelings?";
      ...
      response = "It's wonderful to hear positive momentum in your journey. What specific changes or practices have been most helpful for you?";
      ...
      response = "Thank you for sharing. I'd like to understand more about your experience. What aspects would you like to explore further?";
