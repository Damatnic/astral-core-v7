...
import { Database, TrendingUp, Clock, AlertTriangle, Activity, BarChart3, Zap } from 'lucide-react';
import { useDatabaseMonitoring, QueryMetric, DatabaseInsight } from '@/lib/performance/db-monitoring';
...
const QueryCard: React.FC<QueryCardProps> = ({ query, isExpanded, onToggle }) => {
  ...
  const getOperationColor = (operation: string) => {
    switch (operation) {
      ...
      case 'UPSERT': return 'bg-purple-100 text-purple-800';
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <div className="flex items-center space-x-3 mb-2">
              <span className={`px-2 py-1 rounded text-xs font-medium ${getOperationColor(query.operation)}`}>
                {query.operation}
              </span>
              {query.table && (
            ...
            <div className="flex items-center space-x-4 text-xs text-gray-600">
              ...
              {query.endpoint && (
                <span className="truncate">{query.endpoint}</span>
              )}
            </div>
          </div>
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center space-x-2">
              <span className={`px-2 py-1 rounded text-xs font-medium uppercase ${severityColors[insight.severity]} border`}>
                {insight.severity}
              </span>
...
const DatabaseMonitor: React.FC<DatabaseMonitorProps> = ({
  ...
  const operationOptions = ['all', ...Object.keys(stats.queryDistribution).filter(op => stats.queryDistribution[op as keyof typeof stats.queryDistribution] > 0)];
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Real-time database query performance monitoring
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="p-4 bg-white rounded-lg border">
          ...
          <div className="text-2xl font-bold text-gray-900">{formatDuration(stats.averageDuration)}</div>
        </div>
              ...
              .map(([table, tableStats]) => (
                <div key={table} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-4 text-sm">
                    ...
                    <span className="text-blue-600">{formatDuration(tableStats.averageDuration)} avg</span>
                  </div>
                </div>
              ))}
          </div>
