        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <div className="flex items-center space-x-3 mb-2">
              <span className={`px-2 py-1 rounded text-xs font-medium uppercase ${severityColors[error.severity]} border`}>
                {error.severity}
              </span>
              ...
              <div className="grid grid-cols-2 gap-4 text-xs">
                <div>
                  ...
                  {error.performanceImpact.beforeError.memoryUsage && (
                    <p>Memory: {Math.round(error.performanceImpact.beforeError.memoryUsage.usedJSHeapSize / 1024 / 1024)}MB</p>
                  )}
                ...
                <div>
                  ...
                  {error.performanceImpact.afterError.memoryUsage && (
                    <p>Memory: {Math.round(error.performanceImpact.afterError.memoryUsage.usedJSHeapSize / 1024 / 1024)}MB</p>
                  )}
                </div>
              </div>
            </div>
  ...
  return (
    <div className={`p-4 border rounded-lg ${severityColors[pattern.severity]}`}>
      <div className="flex items-start justify-between mb-3">
        <div>
          <span className={`px-2 py-1 rounded text-xs font-medium uppercase ${severityColors[pattern.severity]} border`}>
            {pattern.severity}
          </span>
          <h3 className="font-medium text-sm text-gray-900 mt-2">
            {pattern.pattern}
          </h3>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 text-xs text-gray-600">
        ...
        <div>
          ...
          <p>{(pattern.count / Math.max(1, (Date.now() - pattern.firstOccurrence) / (24 * 60 * 60 * 1000))).toFixed(1)}/day</p>
        </div>
      </div>
    </div>
  );
...
const ErrorMonitor: React.FC<ErrorMonitorProps> = ({
  ...
  const getSeverityIcon = (severity: string) => {
    switch (severity) {
      ...
      default: return <Bug className="w-4 h-4 text-blue-600" />;
    }
  };
